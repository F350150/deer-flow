...
gateway_client = TestClient(app)

# Mocked LangGraph in-process state store
_IN_PROCESS_THREADS = {}
...
def test_health():
    ...
    resp = gateway_client.get(f"/health")
...
def test_models():
    ...
    resp = gateway_client.get(f"/api/models")
    ...
    if models:
        ...
        resp = gateway_client.get(f"/api/models/{first_model}")
        _assert_status(resp, 200, f"获取模型 '{first_model}'")
        _print_response(resp)

    # 3. 获取不存在的模型 → 404
    _subheader("GET /api/models/nonexistent-model — 不存在的模型 → 404")
    resp = gateway_client.get(f"/api/models/nonexistent-model")
...
def test_mcp():
    ...
    resp = gateway_client.get(f"/api/mcp/config")
...
def test_skills():
    ...
    resp = gateway_client.get(f"/api/skills")
    ...
    if skills:
        ...
        resp = gateway_client.get(f"/api/skills/{first_skill}")
    ...
    resp = gateway_client.get(f"/api/skills/nonexistent-skill")
...
def test_agents():
    ...
    resp = gateway_client.get(f"/api/agents")
    ...
    resp = gateway_client.get(f"/api/agents/check", params={"name": test_agent_name})
    ...
    resp = gateway_client.get(f"/api/agents/{test_agent_name}")
    ...
    resp = gateway_client.get(f"/api/agents/check", params={"name": "bad name!"})
    _assert_status(resp, 422, "非法 Agent 名称")

    # 9. 删除 agent
    _subheader(f"DELETE /api/agents/{test_agent_name} — 删除 Agent")
    resp = gateway_client.delete(f"/api/agents/{test_agent_name}")
...
def test_user_profile():
    ...
    resp = gateway_client.get(f"/api/user-profile")
...
def test_uploads():
    ...
    try:
        ...
        with open(tmp.name, "rb") as f:
            resp = gateway_client.post(f"/api/threads/{thread_id}/uploads", files=[("files", ("test_upload.txt", f, "text/plain"))])
        _assert_status(resp, 200, "上传文件")
        _print_response(resp)

        # 2. 列出上传的文件
        _subheader(f"GET /api/threads/{thread_id}/uploads/list — 列出上传文件")
        resp = gateway_client.get(f"/api/threads/{thread_id}/uploads/list")
...
def test_artifacts():
    ...
    resp = gateway_client.post(f"/api/threads/{thread_id}/uploads", files=[("files", ("artifact_test.txt", content.encode(), "text/plain"))])
...
def test_threads():
    ...
    resp = gateway_client.post(f"/api/threads/{thread_id}/uploads", files=[("files", ("thread_test.txt", b"test data", "text/plain"))])
    _assert_status(resp, 200, "创建线程数据")

    # 删除线程本地数据
    _subheader(f"DELETE /api/threads/{thread_id} — 删除线程本地数据")
    resp = gateway_client.delete(f"/api/threads/{thread_id}")
...
def test_channels():
    ...
    resp = gateway_client.get(f"/api/channels/")
    _assert_status(resp, 200, "获取频道状态")
    _print_response(resp)

    # 2. 重启不存在的频道 (可能 503)
    _subheader("POST /api/channels/nonexistent/restart — 重启不存在的频道")
    resp = gateway_client.post(f"/api/channels/nonexistent/restart")
...
def _lg_create_thread() -> str | None:
    ...
    _IN_PROCESS_THREADS[thread_id] = {"messages": [], "values": {}}
    _info(f"本地线程 ID: {thread_id}")
    return thread_id

def _lg_delete_thread(thread_id: str):
    if thread_id in _IN_PROCESS_THREADS:
        del _IN_PROCESS_THREADS[thread_id]

async def _async_lg_stream_chat(
    thread_id: str,
    message: str,
    *,
    ...
    configurable = {"thinking_enabled": False, "thread_id": thread_id}
    if config_overrides:
    ...
    _subheader(f"Local stream — {label}")
    _info(f"发送消息: '{message[:80]}{'...' if len(message) > 80 else ''}'")
    
    # Store initial message
    if thread_id in _IN_PROCESS_THREADS:
        _IN_PROCESS_THREADS[thread_id]["messages"].append(HumanMessage(content=message))

    try:
        async with make_checkpointer() as checkpointer:
            # Checkpointer is used to store intermediate steps and messages natively
            ...
            # We simulate SSE events by capturing stream from astream()
            async for chunk_mode, chunk in graph.astream(input_data, config=config, stream_mode=modes):
                result["event_count"] += 1
                result["all_events"].append({"mode": chunk_mode, "data": chunk})
                
                # Extract AI output
                if chunk_mode == "values" and isinstance(chunk, dict) and "messages" in chunk:
                    ...
                    if getattr(last_msg, "type", "") == "ai":
                        ...
                        if getattr(last_msg, "tool_calls", None):
                            for tc in last_msg.tool_calls:
                                if tc not in result["tool_calls"]:
                                    result["tool_calls"].append(tc)
                                    
            # Save final state for get_state
            final_state = await graph.aget_state(config)
            if thread_id in _IN_PROCESS_THREADS:
                _IN_PROCESS_THREADS[thread_id]["values"] = final_state.values

        _info(f"收到 {result['event_count']} 个事件")
...
def _lg_stream_chat(*args, **kwargs) -> dict:
    return asyncio.run(_async_lg_stream_chat(*args, **kwargs))

def _lg_get_state(thread_id: str) -> dict | None:
    _subheader(f"GET Local state /threads/{thread_id}/state")
    if thread_id not in _IN_PROCESS_THREADS:
        return None
        
    values = _IN_PROCESS_THREADS[thread_id].get("values", {})
    state = {"values": values}
...
- 框架: LangGraph
"""
    resp = gateway_client.post(f"/api/threads/{thread_id}/uploads", files=[("files", ("project_info.md", file_content.encode("utf-8"), "text/markdown"))])
...
def test_api_docs():
    ...
    resp = gateway_client.get(f"/docs")
